...
export function EnvRowsEditor({
  ...
  const hideRow = (i: number) => {
    setReveal((p) => (p[i] ? { ...p, [i]: false } : p));
  };

  const scheduleHide = (i: number) => {
    if (maskTimers.current[i]) clearTimeout(maskTimers.current[i]);
    maskTimers.current[i] = setTimeout(() => hideRow(i), 900);
  };
        ...
        {value.map((r, i) => (
          <div key={i} className="flex gap-sm items-start">
            ...
            <div className="flex-1 relative">
              ...
              />
              <button type="button" onClick={() => setReveal((p) => ({ ...p, [i]: !p[i] }))} className="absolute right-2 top-1/2 -translate-y-1/2 text-on-surface-variant hover:text-on-surface" title={reveal[i] ? "Hide value" : "Show value"}>
      ...
      {picker && groups && groups.length > 0 && (
        <>
          ...
          <VariablePicker
            ...
            previewText={pickerRow?.value ? `Will resolve to: ${pickerRow.value}` : "Will be resolved at deploy time."}
          />
        </>
      )}
    </div>
