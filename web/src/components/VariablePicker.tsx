...
const SCOPE_META: Record<VarScope, { icon: string; label: string; color: string }> = {
  service: { icon: "dns", label: "Service", color: "text-[#60a5fa]" },
  project: { icon: "folder_copy", label: "Project", color: "text-[#34d399]" },
  organization: { icon: "domain", label: "Organization", color: "text-[#a78bfa]" },
  environment: { icon: "data_object", label: "Environment", color: "text-[#fbbf24]" },
  secrets: { icon: "lock", label: "Secrets", color: "text-[#f472b6]" },
  system: { icon: "memory", label: "System", color: "text-on-surface-variant" },
};
            ...
            <div key={g.scope}>
              ...
              {g.items.map((v) => {
                ...
                return (
                  ...
                  >
                    <span className={`material-symbols-outlined text-[15px] shrink-0 ${SCOPE_META[v.scope].color}`}>{SCOPE_META[v.scope].icon}</span>
