...
export function DatabaseWizard({ open, onClose, fixedProjectId, initialEngine }: { open: boolean; onClose: () => void; fixedProjectId?: string; initialEngine?: string }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Create database" size="lg">
      <form onSubmit={handleSubmit(submit)} className="space-y-lg" noValidate>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-lg">
          ...
          <Field label="Version" hint="Official image tag — empty = default for the engine">
          ...
          <Field label="Database user" hint={errors.user?.message ?? "Empty = aether (auto)"}>
            <Input icon="person" placeholder="aether" {...register("user")} />
          </Field>
          <Field label="Password" hint={errors.password?.message ?? "Empty = auto-generated"}>
            <div className="relative">
              <Input icon="key" type={showPassword ? "text" : "password"} placeholder="••••••••" {...register("password")} />
              <button
                type="button"
                onClick={() => setShowPassword((v) => !v)}
                aria-label={showPassword ? "Hide password" : "Show password"}
                className="material-symbols-outlined absolute right-sm top-1/2 -translate-y-1/2 text-on-surface-variant hover:text-on-surface text-[16px]"
              >
