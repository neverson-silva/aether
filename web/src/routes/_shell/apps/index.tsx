...
import { useAppStates, useDatabases } from "../../../hooks";
import { zodResolver } from "@hookform/resolvers/zod";
import { z } from "zod";
import { Link } from "@tanstack/react-router";
import { useApps, useCreateApp, useProjects } from "../../../hooks";
      ...
      {!!apps?.length || !!databases?.length ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-md">
          ...
          {(apps ?? []).map((app) => {
            ...
            const latestStatus = app.latest_deployment?.status;
            const deploying = latestStatus
              ? ["queued", "building", "starting", "health_checking"].includes(latestStatus)
              : false;
            const pill =
              deploying
                ? { status: "deploying", pulse: true }
                : state === "running"
                  ? { status: "running", pulse: true }
                  : state === "paused"
