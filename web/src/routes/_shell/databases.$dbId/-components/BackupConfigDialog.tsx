...
export function BackupConfigDialog({
  ...
  return (
    <Modal open onClose={onClose} title={existing ? "Edit backup configuration" : "Configure backup"} wide>
      <div className="space-y-lg">
        <div className="space-y-md">
          <Field label="Backup destination">
            <Select value={destinationId} onChange={(e) => setDestinationId(e.target.value)}>
              {destinations.length === 0 && <option value="">No S3 destinations available</option>}
              {destinations.map((d) => (
        ...
        <div className="space-y-md">
          ...
          {scheduleType === "custom" && (
            <Field label="Cron expression" hint={cronOk ? "Valid 5-field crontab expression." : "Invalid cron expression. Expected a valid 5-field crontab expression."}>
