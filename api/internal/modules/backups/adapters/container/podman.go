...
func (p *Podman) ExecIn(ctx context.Context, containerID string, env []string, stdin io.Reader, stdout io.Writer, stderr io.Writer, args ...string) error {
	cmd := exec.CommandContext(ctx, "podman", p.argsWithStdin(containerID, env, args)...)
	cmd.Stdin = stdin
	cmd.Stdout = stdout
	cmd.Stderr = stderr
	return cmd.Run()
...
func (p *Podman) ExecAs(ctx context.Context, containerID string, user string, env []string, stdin io.Reader, args ...string) (string, string, error) {
	cmd := exec.CommandContext(ctx, "podman", p.argsWithStdinAs(containerID, user, env, args)...)
	outBuf := &stringWriter{}
	errBuf := &stringWriter{}
	cmd.Stdin = stdin
	cmd.Stdout = outBuf
...
func (p *Podman) argsWithStdin(containerID string, env []string, args []string) []string {
	return p.argsWithStdinAs(containerID, "", env, args)
}
...
func (p *Podman) argsWithStdinAs(containerID string, user string, env []string, args []string) []string {
	out := []string{"exec", "--interactive"}
	if user != "" {
		out = append(out, "--user", user)
	}
	for _, e := range env {
