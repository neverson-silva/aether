...
func testPool(t *testing.T) *pgxpool.Pool {
	...
	dbCfg := database.Config{
		Host: "127.0.0.1", Port: port, Name: dbName("aether_api_test_snapshots"),
	...
	for _, q := range []string{
		"TRUNCATE snapshot_schedules, snapshots, notification_channels, notifications, alert_events, alert_rules, gitops, compose_apps, templates, backups, databases, autopilot_events, app_policies, workers, cron_jobs, previews, domains, deployments, app_env, env_variables, apps, environments, projects RESTART IDENTITY CASCADE",
