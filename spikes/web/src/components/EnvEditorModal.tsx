...
export function EnvEditorModal({
  ...
  const toggleSecret = (i: number) => {
    setRow(i, { secret: !rows[i].secret, reveal: false, masked: rows[i].secret ? false : false });
  };
  ...
  const parse = (): Record<string, { value: string; secret: boolean }> | null => {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const value = r.secret && r.masked ? (vars?.find((v) => v.key === key)?.value ?? "") : r.value;
  ...
  const bulkImport = () => {
    ...
    for (const raw of lines) {
      ...
      imported.push({ key: m[1], value: m[2].replace(/^"|"$/g, ""), secret: /password|secret|key|token/i.test(m[1]), reveal: false, masked: false });
    }
    if (!failed) {
                ...
                <div key={i} className="flex gap-sm items-start">
                  ...
                  <div className="flex-1 relative">
                    <input
                      ...
                      value={r.secret && r.masked ? "••••••••••••" : r.value}
                      onChange={(e) => handleValueChange(i, e.target.value, e.target)}
                      placeholder="VALUE"
                      spellCheck={false}
                      type={r.secret && !r.reveal && !r.masked ? "password" : "text"}
                    ...
                    {r.secret && (
                      <button
                        type="button"
                        onClick={() => setRow(i, { reveal: !r.reveal, masked: false })}
                  ...
                  <button
                    ...
                    className={`p-2 mt-1 rounded transition-colors ${r.secret ? "text-primary" : "text-on-surface-variant hover:text-on-surface"}`}
                    title={r.secret ? "Secret (masked)" : "Mark as secret"}
                  >
        ...
        {picker && scopeGroups.length > 0 && (
          <>
            ...
            <VariablePicker
              ...
              previewText="Será resolvido durante o deploy."
            />
          </>
        )}
