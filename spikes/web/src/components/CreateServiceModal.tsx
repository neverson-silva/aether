...
import { useCreateApp, useCreateCompose, useProjects, useTrendingTemplates } from "../api/hooks";
...
const createSchema = z
  .object({
    ...
    build_type: z.enum(["dockerfile", "nixpacks", "buildpacks"]).default("dockerfile"),
...
export function CreateServiceModal({
  ...
  const detectFramework = async () => {
    if (!gitURL) {
      toast("Enter a repository URL first", "info");
      return;
    }
    setDetecting(true);
    setDetected(null);
  ...
  return (
    <Modal open={open} onClose={onClose} title="Create Service" wide>
      <form onSubmit={handleSubmit(submit)} className="space-y-lg" noValidate>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-lg">
          <Field label="Project" hint={errors.project_id?.message}>
            <Select {...register("project_id")} disabled={!!fixedProjectId}>
          ...
          <Field label="Name" hint={errors.name?.message}>
            <Input icon="label" placeholder="ex: api-gateway" {...register("name")} />
          </Field>
          <Field label="Source">
            <Select {...register("source_type")}>
              <option value="image">OCI Image</option>
              <option value="git">Git Repository</option>
              <option value="compose">Docker Compose</option>
              <option value="template">Template</option>
            </Select>
          </Field>
          <Field label="Container port" hint={errors.port?.message}>
            <Input icon="counter_1" type="number" {...register("port")} />
          </Field>
        </div>
        ...
        {source === "template" ? (
          ...
          <div className="relative">
            <Field label="Image" hint={errors.image?.message || "ex: nginx:alpine"}>
              <Input icon="image" placeholder="registry/repo:tag" {...register("image")} onFocus={() => setImageSuggest(true)} onBlur={() => setTimeout(() => setImageSuggest(false), 150)} />
            </Field>
            {imageSuggest && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-lg">
            <div className="md:col-span-2">
              <Field label="Repository URL" hint={errors.git_url?.message}>
                <Input icon="code" placeholder="git@github.com:org/repo.git" {...register("git_url")} />
              </Field>
            </div>
            <div className="flex items-end">
              <Button type="button" variant="ghost" disabled={detecting || !gitURL} onClick={detectFramework}>
                <span className="material-symbols-outlined text-[16px]">{detecting ? "hourglass_top" : "radar"}</span>
                {detecting ? "Detecting..." : "Detect framework"}
              </Button>
            </div>
            <Field label="Branch" hint={errors.git_branch?.message}>
              <Input icon="fork_right" placeholder="main" {...register("git_branch")} />
            </Field>
            <div className="md:col-span-2">
              <Field label="Dockerfile" hint={errors.dockerfile?.message}>
                <Input icon="description" placeholder="Dockerfile" {...register("dockerfile")} />
              </Field>
            </div>
            <Field label="Build method">
              <Select {...register("build_type")}>
          ...
          <div className="bg-surface-container-low border border-primary/30 rounded-lg p-md space-y-sm">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-md">
              <div>
                ...
                <p className="font-code-md text-code-md text-on-surface">{detected.build_method || "nixpacks"}</p>
              </div>
              <div>
        ...
        <details className="group border border-outline-variant rounded-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-lg">
          <Field label="Memory (MiB)" hint={errors.mem_mb?.message || "0 = unlimited"}>
            <Input icon="memory" type="number" {...register("mem_mb")} />
          </Field>
          <Field label="Health check path" hint={errors.health_path?.message}>
            <Input icon="monitor_heart" placeholder="/" {...register("health_path")} />
          </Field>
        </div>
