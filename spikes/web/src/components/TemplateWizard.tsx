...
import { useCreateApp, useProjects, useTemplateCategories, useTemplatesFiltered } from "../api/hooks";
import type { TemplateItem } from "../api/hooks";
...
export function TemplateWizard({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const create = async () => {
    ...
    try {
      ...
      toast(`Template "${selected.name}" created as "${name}"`);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {(templates ?? []).map((t) => (
                  ...
                  >
                    ...
                    <div>
                      <div className="w-8 h-8 rounded flex items-center justify-center mb-3" style={{ background: iconBg(t), color: iconColor(t) }}>
                        <TechIcon name={t.icon} size={18} className="" />
                      </div>
                      <h3 className="font-body-md text-body-md font-semibold text-on-surface truncate">{t.name}</h3>
              ...
              <details className="group border border-outline-variant rounded-lg">
                ...
                <div className="px-md pb-md pt-lg border-t border-outline-variant/60 flex flex-col gap-lg">
                  ...
                  <div>
                    ...
                    <p className="font-code-md text-[10px] text-on-surface-variant/50 mt-sm">Quota máxima aplicada ao volume persistente (storage-opt size). 0 = sem limite.</p>
                  </div>
                </div>
              </details>
            </div>
