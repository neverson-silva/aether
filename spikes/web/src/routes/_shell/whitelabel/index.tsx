...
import { useBranding, useSaveBranding } from "../../../api/hooks";
import {
  Button,
  Card,
  Field,
...
export function Whitelabel() {
  ...
  const submit = async (values: z.infer<typeof brandSchema>) => {
    try {
      ...
      toast("Branding saved and applied");
    } catch (err) {
  ...
  return (
    <div className="space-y-lg max-w-[1080px]">
      <AppPageHeader
        title="Whitelabeling"
        description="Configure platform branding for your tenant. The primary color is applied to the shell."
      />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-lg">
          ...
          <Card>
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-lg">
                <Field label="Platform Name" hint={errors.name?.message}>
                  <Input icon="badge" placeholder="Aether" defaultValue={branding?.name} {...register("name")} />
                </Field>
                <Field label="Logo URL" hint={errors.logo_url?.message || "leave empty for the default logo"}>
                  <Input icon="image" placeholder="https://cdn.example.com/logo.svg" defaultValue={branding?.logo_url} {...register("logo_url")} />
                </Field>
                <Field label="Primary Color" hint={errors.primary_color?.message}>
                  <div className="flex items-center gap-sm">
                    ...
                    />
                    <Input icon="palette" placeholder="#568dff" defaultValue={branding?.primary_color} {...register("primary_color")} />
                  </div>
                </Field>
                <Field label="Accent Color" hint={errors.accent_color?.message}>
                  <Input icon="palette" placeholder="#568dff" defaultValue={branding?.accent_color} {...register("accent_color")} />
                </Field>
        ...
        <div className="space-y-lg">
          <Card className="flex flex-col items-center text-center">
            ...
            >
              <span className="material-symbols-outlined" style={{ color: "#002d6f" }}>
                cloud
              </span>
            </div>
          ...
          <Card>
            ...
            <div className="space-y-sm">
              <div className="flex items-center justify-between p-sm rounded border border-outline-variant/60">
                ...
                <StatusPill status={branding?.name ? "active" : "disabled"} />
              </div>
              <div className="flex items-center justify-between p-sm rounded border border-outline-variant/60">
