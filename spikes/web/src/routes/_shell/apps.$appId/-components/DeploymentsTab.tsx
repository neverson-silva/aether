...
import { useDeployCompare, useDeploymentLog } from "../../../../api/hooks";
import { Button, Card, Modal, StatusPill, cn, DeploymentStatus, isDeploymentActive, useToast } from "../../../../components/ui";
...
export function suggestFix(error: string): { level: "error" | "warning"; title: string; detail: string } | null {
  ...
  const checks: { m: RegExp; level: "error" | "warning"; title: string; detail: string }[] = [
    ...
    { m: /dockerfile.*not found|dockerfile.*missing|no dockerfile/i, level: "warning", title: "Dockerfile missing", detail: "No Dockerfile was found in the repository. Switch the build method to Nixpacks, or add a Dockerfile to the repo." },
...
export function DeploymentsTab({ appId, deployments, onRollback }: { appId: string; deployments: DeploymentRow[]; onRollback: () => void }) {
  ...
  const log = useDeploymentLog(appId, logDep);
  ...
  const toggle = (id: string) => {
    setSel((prev) => {
      ...
      if (next.has(id)) next.delete(id);
        ...
        if (next.size >= 2) {
          toast("Select exactly two deployments to compare", "info");
          return prev;
        }
        next.add(id);
      }
  ...
  return (
    <>
      <Card>
        <div className="flex items-center justify-between mb-md">
          ...
          <div className="flex items-center gap-sm">
            {sel.size === 2 && (
              <Button variant="subtle" onClick={() => { const [a, b] = [...sel]; setComparePair({ a, b }); }}>
                  ...
                  >
                    <td className="px-sm py-2">
                      <input
                        ...
                        className="w-3.5 h-3.5 rounded-sm bg-surface border-outline-variant text-primary"
                      />
                    </td>
      ...
      <Modal open={!!comparePair} onClose={() => setComparePair(null)} title="Deploy comparison" wide>        {compare.data ? (
          ...
          <p className="font-body-sm text-body-sm text-on-surface-variant">Loading comparison...</p>
        )}
      </Modal>

      <Modal open={!!logDep} onClose={() => setLogDep(null)} title={`Deployment #${log.data?.number ?? ""} log`} wide>
        {log.data ? (
          <div>
            {log.data.error && (
