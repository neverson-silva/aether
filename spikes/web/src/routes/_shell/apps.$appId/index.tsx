    ...
    <div className="space-y-lg">
        ...
        <Card>
          ...
          {stats ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-md">
              <Metric label="CPU" value={`${stats.stats.CPUPercent.toFixed(2)}%`} icon="speed" />
              <Metric label="Memory" value={fmtBytes(stats.stats.MemBytes)} icon="memory" />
              <Metric label="Limit" value={fmtBytes(stats.stats.MemLimit)} icon="data_usage" />
              <Metric label="PIDs" value={String(stats.stats.Pids)} icon="track_changes" />
            </div>
          ) : (
      ...
      {tab === "settings" && (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-lg">
          ...
          <Card>
            <div className="flex items-center justify-between mb-md">
              <h2 className="font-label-caps text-label-caps text-on-surface-variant uppercase">Environment Variables</h2>
              ...
              {(detail?.env ?? []).map((e) => (
                <div key={e.name} className="flex items-center justify-between gap-sm p-sm rounded border border-outline-variant/60">
                  ...
                  <div className="flex items-center gap-sm shrink-0">
                    ...
                    <button
                      onClick={() => deleteEnv.mutate(e.name, { onSuccess: () => toast("Variable removed") })}
              ...
              {(domains ?? []).map((d) => (
                <div key={d.id} className="flex items-center justify-between gap-sm p-sm rounded border border-outline-variant/60">
                  <div className="min-w-0">
                    ...
                    {d.https && <StatusPill status={d.cert_status} />}
                  </div>
                  <button
                    onClick={() => removeDomain.mutate(d.host, { onSuccess: () => toast("Domain removed") })}
