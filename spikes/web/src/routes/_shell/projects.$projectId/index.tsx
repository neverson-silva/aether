...
import { z } from "zod";
import type { EnvSummary } from "../../../api/hooks";
import {
  useProjects,
  useEnvironments,
...
function ProjectDetail() {
  ...
  const envApps = (apps ?? []).filter((a) => a.environment_id === selected);
  ...
  return (
    <div className="space-y-lg">
        ...
        <>
          ...
          <div className="bg-surface-container-low border border-outline-variant rounded-lg">
            {allServices.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-sm p-sm">
                {allServices.map((svc) => {
                  ...
                  const target = isDb ? `/databases/${svc.id}` : `/apps/${svc.id}`;
                  const sub = isDb ? `${(svc as { engine?: string }).engine} ${(svc as { version?: string }).version}` : (svc as { image?: string }).image ?? "";
                  return (
                    <Link
                      key={svc.type + svc.id}
                      to={target}
                      className="group flex items-center gap-sm px-sm py-2.5 rounded bg-surface-container-lowest border border-outline-variant/50 hover:border-primary/50 hover:bg-surface-container-high/40 transition-colors cursor-pointer min-w-0"
                      title={`Open ${svc.name}`}
                    >
                      <TechIcon name={isDb ? (svc as { engine?: string }).engine : (svc as { source?: string }).source === "git" ? "gitlab" : "docker"} size={16} className="text-primary shrink-0" />
                      <span className="flex-1 min-w-0">
                        <span className="block font-body-md text-body-md text-on-surface truncate">{svc.name}</span>
                        <span className="block font-code-md text-code-md text-on-surface-variant/60 truncate">{sub} · :{svc.port}</span>
                      </span>
                      <StatusPill status={pill} pulse={pulse} />
                      <span className="material-symbols-outlined text-[16px] text-on-surface-variant/30 group-hover:text-primary transition-colors shrink-0">chevron_right</span>
                    </Link>
                  );
                })}
              </div>
      ...
      <CreateServiceLauncher open={createOpen} onClose={() => setCreateOpen(false)} fixedProjectId={projectId} />
