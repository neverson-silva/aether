    ...
    <BaseDrawer.Root>
      ...
      <BaseDrawer.Portal>
        ...
        <BaseDrawer.Viewport className="fixed inset-0 z-50 flex">
          <BaseDrawer.Popup
            className={`m-0 flex h-full w-[min(22rem,calc(100vw-1rem))] flex-col border-border bg-surface-modal p-5 text-foreground shadow-xl backdrop-blur-xl transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] sm:p-6 ${position}`}
          >
            {title ? (
            ...
            <BaseDrawer.Close className="mt-4 self-end rounded-xl border border-border px-3 py-2 text-body-sm outline-none transition-[background-color,border-color,transform] duration-150 hover:bg-surface-container focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.985]">
              Close
            </BaseDrawer.Close>
          </BaseDrawer.Popup>
        </BaseDrawer.Viewport>
