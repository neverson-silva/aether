...
export function LogViewer({
  ...
  const copyVisibleLogs = async () => {
    try {
      await navigator.clipboard.writeText(
        visible.map((line) => line.message).join("\n"),
      );
      setCopied(true);
  ...
  return (
    <section
      className={`flex min-h-0 flex-col overflow-hidden bg-surface-lowest ${fullHeight ? "h-full flex-1" : "rounded-2xl border border-border shadow-sm"}`}
    >
      <header className="flex min-h-12 shrink-0 flex-wrap items-center gap-2 border-b border-border bg-surface-container-low/80 px-4 py-3">
        <div className="flex min-w-48 max-w-2xl flex-1 items-center gap-2 rounded-xl border border-border bg-surface-control px-3 transition-[border-color,box-shadow,background-color] duration-200 focus-within:border-primary focus-within:bg-surface-card focus-within:ring-2 focus-within:ring-primary/20">
        ...
        <button
          ...
          className="inline-flex shrink-0 items-center gap-1 rounded-xl border border-transparent px-3 py-2 text-body-sm text-muted-foreground outline-none transition-[border-color,background-color,color,transform] duration-150 hover:border-border hover:bg-surface-container hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.985]"
        >
        ...
        <button
          ...
          className="inline-flex shrink-0 items-center gap-1 rounded-xl border border-transparent px-3 py-2 text-body-sm text-muted-foreground outline-none transition-[border-color,background-color,color,transform] duration-150 hover:border-border hover:bg-surface-container hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.985] disabled:cursor-not-allowed disabled:opacity-50"
        >
