...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input({
  ...
  const control = (
    <div
      className={`flex items-center gap-1 rounded-lg border border-border bg-surface-control shadow-sm transition-[background-color,border-color,box-shadow] duration-200 hover:bg-surface-container-highest/40 focus-within:!border-primary focus-within:ring-2 focus-within:ring-ring/20 group-data-[invalid]:border-status-danger group-data-[invalid]:focus-within:!border-status-danger group-data-[invalid]:focus-within:ring-status-danger/20 ${error ? 'border-status-danger focus-within:!border-status-danger focus-within:ring-status-danger/20' : ''}`}
    >
      {LeadingIcon ? (
        <LeadingIcon
