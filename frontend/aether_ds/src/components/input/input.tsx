...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input({
  ...
  const control = (
    <div
      className={`flex items-center gap-1 rounded-md border border-border bg-surface-control transition-colors hover:bg-surface-container-highest/40 focus-within:!border-primary focus-within:ring-2 focus-within:ring-ring/20 group-data-[invalid]:border-status-danger group-data-[invalid]:focus-within:!border-status-danger group-data-[invalid]:focus-within:ring-status-danger/20 ${error ? 'border-status-danger focus-within:!border-status-danger focus-within:ring-status-danger/20' : ''}`}
    >
      {LeadingIcon ? (
        <LeadingIcon
