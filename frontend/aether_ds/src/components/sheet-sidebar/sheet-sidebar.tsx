    ...
    <Drawer.Root>
      ...
      <Drawer.Portal>
        ...
        <Drawer.Viewport className="fixed inset-0 z-50 flex">
          <Drawer.Popup
            className={`m-0 flex h-full w-[min(22rem,calc(100vw-1rem))] flex-col border-border bg-surface-card p-5 shadow-xl transition-transform duration-300 ease-[cubic-bezier(0.2,0.8,0.2,1)] sm:p-6 ${side === "right" ? "ml-auto border-l data-[starting-style]:translate-x-full data-[ending-style]:translate-x-full" : side === "bottom" ? "mt-auto h-auto w-full border-t data-[starting-style]:translate-y-full data-[ending-style]:translate-y-full" : "border-r data-[starting-style]:-translate-x-full data-[ending-style]:-translate-x-full"}`}
          >
            ...
            <Drawer.Close className="mt-4 self-end rounded-xl border border-border px-3 py-2 text-body-sm outline-none transition-[background-color,transform] duration-150 hover:bg-surface-container focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.985]">
              Close
            </Drawer.Close>
          </Drawer.Popup>
        </Drawer.Viewport>
