...
function useServers() {
  return useQuery({
    queryKey: ["servers"],
    queryFn: () => apiGet<ServerInfo[]>("/api/v1/servers"),
  });
}
function fmtMem(n: number) {
  if (n <= 0) return "—";
...
function Servers() {
  ...
  const [token, setToken] = useState<{ token: string; core: string } | null>(
    null,
  );
  const [name, setName] = useState("");
  const createToken = useMutation({
    mutationFn: () =>
      apiPost<{ token: string; core: string }>("/api/v1/servers/token", {
        name,
      }),
  });
  ...
  const generate = () => {
    if (!name.trim()) {
      add({
        ...
        description: "Give the server a name first.",
        tone: "error",
      });
      return;
    }
  ...
  return (
    <main className="space-y-lg">
      <header className="relative isolate overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-primary/10 via-surface-card to-secondary/5 px-lg py-xl shadow-sm sm:flex sm:items-end sm:justify-between">
        <div>
          <p className="font-label-caps text-label-caps uppercase text-primary">Infrastructure</p>
        ...
        <Dialog
          ...
          title="Add server"
          trigger={
            <Button icon={Plus as unknown as DesignIcon}>Add server</Button>
          }
        >
            ...
            {!token ? (
              <>
                ...
                <div className="flex justify-end">
                  <Button loading={createToken.isPending} onClick={generate}>
                    Generate token
                  </Button>
                </div>
              </>
              ...
              <>
                <p className="text-body-sm text-muted-foreground">
                  Run this command on the remote server to enroll it:
                </p>
                <CodeBlock
                  code={`aether agent --core ${token.core} \\n  --token ${token.token} \\n  --name ${name.trim() || "worker"}`}
                  language="shell"
                />
                <div className="flex justify-end">
      ...
      <Card variant="elevated">
        ...
        <p className="mt-2 text-body-sm text-muted-foreground">
          The core embeds its own runtime. Deploys without a server assignment
          run here.
        </p>
      </Card>
      {query.error ? (
        <InlineError
          ...
          message="Try again to refresh registered agents."
      ...
      <Card variant="elevated" padding="none">
            ...
            <tbody className="divide-y divide-border">
              {query.isLoading ? (
                ...
                (query.data ?? []).map((server) => (
                  <tr key={server.id} className="hover:bg-surface-container">
                    <td className="px-3 py-3">
                      <div className="text-body-md text-foreground">
                        {server.name}
                      </div>
                      <div className="font-mono text-code-md text-muted-foreground">
                        {server.id}
                      </div>
                    </td>
                    <td className="px-3 py-3">
                    ...
                    <td className="px-3 py-3 font-mono text-code-md text-muted-foreground">
                      {fmtMem(server.mem_total_bytes)}
                    </td>
                    <td className="px-3 py-3">
                      <div className="flex items-center gap-2">
                        <div className="h-1.5 w-16 overflow-hidden rounded bg-surface-container">
                          <div
                            className="h-full rounded bg-primary"
                            style={{
                              width: `${Math.min(100, Math.round(server.load * 20))}%`,
                            }}
                          />
                        </div>
        ...
        {!query.isLoading && !query.data?.length ? (
          <EmptyState
            ...
            description="Add a server to run remote deployments."
          />
        ) : null}
      </Card>
    </main>
