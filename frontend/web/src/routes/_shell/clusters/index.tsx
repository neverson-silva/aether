...
function Clusters() {
  ...
  const [join, setJoin] = useState<{ cluster: string; name: string } | null>(
    null,
  );
  const {
    register,
  ...
  const submit = async (values: z.infer<typeof schema>) => {
    try {
      ...
      add({
        ...
        description:
          err instanceof Error ? err.message : "Unable to create cluster.",
        tone: "error",
      });
    }
  };

  const unassigned = (servers ?? []).filter(
    (s) => !s.cluster_id || s.cluster_id === "",
  );
    ...
    <div className="space-y-lg">
        ...
        <Card key={c.id}>
          <div className="flex items-center justify-between mb-md">
            <div className="flex items-center gap-sm">
              ...
              <h2 className="font-body-md text-body-md text-on-surface">
                {c.name}
              </h2>
              ...
              {c.labels.map((l) => (
                <span
                  key={l}
                  className="rounded-full border border-outline-variant px-1.5 py-0.5 font-code-md text-code-md text-primary"
                >
                  {l}
                </span>
              ))}
            ...
            <div className="flex items-center gap-sm">
              <Badge tone={c.servers.length > 0 ? "success" : "neutral"} dot>
                {c.servers.length > 0 ? "Active" : "Disabled"}
              </Badge>
              <Button
                variant="ghost"
                onClick={() => setJoin({ cluster: c.id, name: c.name })}
              >
                <Plus size={16} />
                Add server
              </Button>
          ...
          {c.servers.length === 0 ? (
            <EmptyState
              title="No servers in this cluster yet"
              description="Deploys targeting it stay local until a node joins."
            ...
            <div className="space-y-sm">
              {c.servers.map((srv) => (
                <div
                  key={srv.id}
                  className="flex flex-wrap items-center justify-between gap-sm rounded-xl border border-outline-variant/60 p-sm"
                >
                  <div className="flex items-center gap-sm">
                    <Badge
                      tone={srv.status === "healthy" ? "success" : "neutral"}
                      dot
                    >
                      {srv.status}
                    </Badge>
      ...
      {(clusters ?? []).length === 0 && (
        <EmptyState
          ...
          description="Group your agents for affinity-aware scheduling."
          action={
      ...
      <Dialog
        ...
        trigger={<span />}
        onOpenChange={(value) => {
          if (!value) setJoin(null);
        }}
        title={`Add server to ${join?.name ?? ""}`}
      >
        {join && (
          <div className="space-y-lg">
            <Field label="Server">
            ...
            {unassigned.length === 0 && (
              <EmptyState
                title="No unassigned servers available"
                className="border-0 p-4"
              />
            )}
          </div>
