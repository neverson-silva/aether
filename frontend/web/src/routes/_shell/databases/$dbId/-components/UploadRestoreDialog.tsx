...
export function UploadRestoreDialog({
  ...
  const job: RestoreJob | null =
    (finalRestore.data &&
    ["uploading", "validating", "ready", "queued"].includes(
      finalRestore.data.status,
    )
      ? selected
    ...
    cancelRestore.mutate(selected.id, {
      onSuccess: () => {
        ...
        queryClient.invalidateQueries({
          queryKey: ["database-restore-jobs", "service", dbId],
        });
        onClose();
      },
      onError: (e) =>
  ...
  return (
    <Dialog
      ...
      title="Import backup from file"
    >
      <div className="space-y-lg">
        {error && (
          <div className="rounded-2xl border border-error/40 bg-error/10 p-md font-body-md text-body-md text-error shadow-sm">
            {error}
          </div>
        )}
        ...
        {phase === "pick" && (
          <div
            className="cursor-pointer rounded-2xl border-2 border-dashed border-outline-variant bg-surface-card p-xl text-center shadow-sm outline-none transition-[border-color,background-color,box-shadow] duration-200 hover:border-primary/60 hover:bg-surface-container-low focus-visible:ring-2 focus-visible:ring-ring"
            onDrop={drop}
          ...
          >
            ...
            <p className="font-body-md text-body-md text-on-surface mt-md">
              Drop a dump file here or click to browse
            </p>
            <p className="font-body-sm text-body-sm text-on-surface-variant mt-0.5">
              Supported: Postgres (.dump/.backup/.sql/.sql.gz), MySQL/MariaDB
              (.sql/.sql.gz), SQL Server (.bak), Oracle (.dmp)
            </p>
          </div>
        )}
        ...
        {phase === "ready" && job && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-md">
            <div className="rounded-xl border border-outline-variant/60 bg-surface-container-low/40 px-md py-sm">
              <span className="font-label-caps text-label-caps text-on-surface-variant uppercase block mb-0.5">
                File
              </span>
              <span className="font-code-md text-code-md text-on-surface break-all">
                {job.source_filename}
              </span>
            </div>
            <div className="rounded-xl border border-outline-variant/60 bg-surface-container-low/40 px-md py-sm">
        ...
        {phase === "ready" && job && (
          <>
            <div className="rounded-2xl border border-warning/40 bg-warning/10 p-md shadow-sm">
              <p className="font-body-md text-body-md text-on-surface font-medium">
                Restore database?
              </p>
              <p className="font-body-sm text-body-sm text-on-surface-variant mt-sm">
                This will replace the current data in {dbName ?? "the database"}{" "}
                with the contents of {job.source_filename}. This action may
                cause temporary downtime. Type the database name to continue.
              </p>
            </div>
        ...
        {phase === "restoring" && running && (
          <div className="flex items-center gap-sm rounded-2xl border border-primary/40 bg-primary/10 p-md text-on-surface shadow-sm">
            <CircleNotch size={18} className="animate-spin text-primary" />
            <span className="font-body-md text-body-md">
              Restore in progress: {job?.status}. Keep this window open.
            </span>
          </div>
        )}
        ...
        {done && job && (
          <div
            className={`flex items-center gap-sm rounded-2xl border p-md shadow-sm ${job.status === "completed" ? "border-success/40 bg-success/10 text-success" : "border-error/40 bg-error/10 text-error"}`}
          >
