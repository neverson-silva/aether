...
function projectStatus(statuses: string[]): {
  label: string;
  ...
  if (
    normalized.some((status) =>
      ["failed", "cancelled", "rolled_back", "error", "dead"].includes(status),
    )
  ) {
  ...
  if (
    normalized.some((status) =>
      ["healthy", "ready", "running", "active"].includes(status),
    )
  ) {
...
function EnvModal({
  ...
  return (
    <Dialog
      ...
      title={initial ? "Edit environment" : "Create environment"}
    >
      <form
      ...
      >
        ...
        <Field
          ...
          description="Hex color used in the selector, for example #ff8800."
        >
...
function ProjectDetail() {
  ...
  const validEnvironments = useMemo(
    () =>
      (envs ?? []).filter((environment) =>
        Boolean(environment?.id && environment?.name?.trim()),
      ),
    [envs],
  );
  const selectedId = (() => {
    if (!validEnvironments.length) return null;
    const fromUrl = validEnvironments.find(
      (e) => e.slug === search.environment,
    );
    if (fromUrl) return fromUrl.id;
    const stored = localStorage.getItem(`aether.env.${projectId}`);
    const fromStorage = validEnvironments.find(
      (e) => e.id === stored || e.slug === stored,
    );
    if (fromStorage) return fromStorage.id;
    return (
      validEnvironments.find((e) => e.is_default)?.id ?? validEnvironments[0].id
    );
  })();
  ...
  const allServices = (servicesCatalog.data ?? []).map((service) => ({
    ...
    runtimeLabel:
      service.status === "pending" ? "Pending deployment" : undefined,
  }));
  const serviceKeys = allServices
    .map((service) => `${service.type}:${service.id}`)
    .join(",");
  const selectedServices = allServices.filter((service) =>
    selectedServiceKeys.has(`${service.type}:${service.id}`),
  );
  ...
  const runBulk = async (operation: "delete" | "deploy") => {
    ...
    if (failures.length)
      setBulkError(`Could not ${operation} ${failures.join(", ")}.`);
  ...
  const confirmDeleteEnv = async () => {
    ...
    try {
      ...
      const remaining = validEnvironments.filter(
        (e) => e.id !== confirmDelete.id,
      );
      if (remaining?.length)
        selectEnv(remaining.find((e) => e.is_default)?.id ?? remaining[0].id);
    } catch (err) {
      add({
        title: "Environment deletion failed",
        description:
          err instanceof Error ? err.message : "Unable to delete environment.",
        tone: "error",
      });
    }
  };
  ...
  return (
    <main className="mx-auto flex w-full max-w-screen-2xl flex-col gap-lg p-6 lg:p-8">
      <section className="relative isolate overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-primary/10 via-surface-card to-secondary/5 px-lg py-xl shadow-md">
        <div className="pointer-events-none absolute -right-16 -top-20 size-64 rounded-full bg-primary/10 blur-3xl" />
        <div className="relative flex flex-wrap items-end justify-between gap-lg">
          <div className="min-w-0 flex-1">
            <p className="font-label-caps text-label-caps uppercase text-primary">
              Workspace
            </p>
            <h1 className="mt-sm truncate font-display-lg text-display-lg tracking-[-0.04em] text-on-surface">
              {project.name}
            </h1>
            <p className="mt-sm text-body-md text-on-surface-variant">
              {validEnvironments.length} environment
              {validEnvironments.length === 1 ? "" : "s"} · Services, delivery
              and runtime state
            </p>
          </div>
          <EnvironmentSwitcher
            options={validEnvironments.map((environment) => {
              const environmentServices = (
                projectServicesCatalog.data ?? []
              ).filter(
                (service) => service.environment_id === environment.id,
              ).length;
            ...
            onEdit={(id) => {
              const environment = validEnvironments.find(
                (item) => item.id === id,
              );
              if (environment)
                setModal({
          ...
          <ActionsMenu
            ...
            onRename={() =>
              ...
              setModal({
                ...
                editing: {
                  id: activeEnv.id,
                  name: activeEnv.name,
                  description: activeEnv.description,
            ...
            onSetDefault={() =>
              activeEnv &&
              setDefault.mutate(activeEnv.id, {
                onSuccess: () =>
                  add({
                    title: "Default environment updated",
                    tone: "success",
            ...
            onDelete={() =>
              activeEnv &&
              setConfirmDelete({ id: activeEnv.id, name: activeEnv.name })
            }
          />
        </div>
      </section>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Create an environment to organize service deployments."
        />
      ) : (
        <>
          <div className="grid grid-cols-1 gap-md md:grid-cols-3">
            <Card variant="elevated" padding="md">
              ...
              <p className="mt-xs font-display-md text-headline-sm tracking-[-0.025em] text-on-surface">
                {allServices.length}
              </p>
            </Card>
            <Card variant="elevated" padding="md">
          ...
          {allServices.length === 0 ? (
            <EmptyState
              title="No services in this environment yet"
              description="Create one in Services."
            />
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-md">
              {allServices.map((svc) => {
                ...
                const sub = isCompose
                  ...
                  : isDb
                    ? `${(svc as { engine?: string }).engine} ${(svc as { version?: string }).version}`
                    : ((svc as { image?: string }).image ?? "");
                ...
                return (
                  <div key={serviceKey} className="relative min-w-0">
                    ...
                    <a
                      ...
                      className="group flex min-w-0 flex-col items-start gap-sm rounded-xl border border-border bg-surface-card px-md pb-10 pt-5 shadow-sm transition-[background-color,border-color,box-shadow,transform] duration-200 hover:-translate-y-0.5 hover:border-primary/50 hover:bg-surface-container-high/40 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                      title={`Open ${svc.name}`}
                    >
                      <span className="flex w-full min-w-0 items-center justify-between">
                        <TechIcon
                          name={
                            isCompose
                              ? "docker"
                              : isDb
                                ? (svc as { engine?: string }).engine
                                : (svc as { source?: string }).source === "git"
                                  ? "gitlab"
                                  : "docker"
                          }
                          size={16}
                        ...
                        <RuntimeStatus
                          ...
                          label={svc.runtimeLabel}
                          live={isRuntimeLive(svc.runtimeStatus)}
                        />
                      </span>
                      <span className="w-full min-w-0">
                        ...
                        <span className="block truncate font-code-md text-code-md text-on-surface-variant/60">
                          {sub}
                          {!isCompose && ` · :${svc.port}`}
                        </span>
                      </span>
                      <span className="self-end text-on-surface-variant/30 transition-colors group-hover:text-primary">
                        →
                      </span>
                    </a>
                  </div>
      ...
      <CreateServiceLauncher
        ...
        fixedEnvironmentId={selected ?? undefined}
        onCreated={(serviceId) => {
          window.location.href = `/apps/${serviceId}?returnTo=${encodeURIComponent(window.location.pathname + window.location.search)}`;
        }}
      />
      <DatabaseWizard
        open={dbCreateOpen}
        onClose={() => setDbCreateOpen(false)}
        fixedProjectId={projectId}
        fixedEnvironmentId={selectedId ?? undefined}
        onCreated={(databaseId) => {
          window.location.href = `/apps/${databaseId}?returnTo=${encodeURIComponent(window.location.pathname + window.location.search)}`;
        }}
      />
    ...
    <div ref={ref} className="relative inline-block">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 z-[70] w-64 rounded-xl bg-surface-popover border border-border-subtle shadow-md overflow-hidden p-1.5">
          ...
          <p className="px-2.5 pt-3 pb-1 font-label-caps text-label-caps text-on-surface-variant/60 uppercase">
            Project
          </p>
          ...
          {!activeEnv?.is_default &&
            item(Star, "Set as Default", onSetDefault, false, !hasEnv)}
