        ...
        <Card className="xl:col-span-2">
          ...
          <div className="overflow-x-auto rounded-2xl border border-outline-variant bg-surface-container-low">
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-outline-variant text-label-caps text-on-surface-variant">
                  <th className="px-sm py-2">Name</th>
                  <th className="px-sm py-2">Repo</th>
                  <th className="px-sm py-2">Branch</th>
                  <th className="px-sm py-2">Status</th>
                  <th className="px-sm py-2">Drift</th>
                  <th className="px-sm py-2">Sync</th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {isLoading && (
                  <tr>
                    <td colSpan={7} className="px-sm py-sm">
                      <Skeleton
                        variant="table"
                        aria-label="Loading GitOps configurations"
                      />
                    </td>
                  </tr>
                )}
                {(configs ?? []).map((g) => (
                  ...
                  >
                    ...
                    <td className="px-sm py-2">
                      <Badge
                        tone={
                          g.last_status === "apply_failed"
                            ? "danger"
                            : g.last_status === "applied"
                              ? "success"
                              : "neutral"
                        }
                      >
                        {statusTone(g.last_status)}
                      </Badge>
                    </td>
                    <td className="px-sm py-2 font-code-md text-code-md text-on-surface-variant">
                    ...
                    <td className="px-sm py-2 font-code-md text-code-md text-on-surface-variant/60">
                      {g.last_sync
                        ? new Date(g.last_sync).toLocaleString()
                        : "—"}
                    </td>
                    <td className="px-sm py-2 text-right space-x-sm">
                      <button
                        onClick={() =>
                          sync.mutate(g.id, {
                            onSuccess: () =>
                              add({ title: "Sync complete", tone: "success" }),
                          })
                        }
            ...
            {(configs ?? []).length === 0 && !isLoading && (
              <EmptyState
                title="No GitOps configs yet"
                description="Create a configuration to manage deployments from Git."
                className="border-0"
              />
            )}
          </div>
          ...
          <Field label="Apply mode">
            <Select
              ...
              options={[
                {
                  value: "manual",
                  label: "Manual (review drift, apply via sync)",
                },
                { value: "auto", label: "Auto (apply on every poll)" },
              ]}
            />
          </Field>
