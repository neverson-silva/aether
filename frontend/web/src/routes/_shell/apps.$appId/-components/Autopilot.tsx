...
import { Badge, Button, Card, Checkbox, Field, Input, Skeleton, useToast } from "@aether/design-system";
...
export function Autopilot({ appID }: { appID: string }) {
  ...
  if (isLoading) return <div className="space-y-sm" aria-label="Loading autopilot settings"><Skeleton variant="card" /><Skeleton variant="card" /></div>;
  ...
  const submit = async (values: z.infer<typeof schema>) => {
    try {
      ...
      add({ title: "Could not save policy", description: err instanceof Error ? err.message : "Try again later.", tone: "error" });
    }
  };
    ...
    <Card>
      <div className="flex items-center justify-between mb-md">
        <div className="flex items-center gap-sm">
          ...
          <Badge tone={policy?.enabled ? "success" : "neutral"}>{policy?.enabled ? "Active" : "Disabled"}</Badge>
        </div>
      ...
      {!open ? (
        ...
        <form onSubmit={handleSubmit(submit)} className="grid grid-cols-2 md:grid-cols-4 gap-lg">
          <label className="flex items-center gap-sm col-span-4 cursor-pointer select-none">
            <Checkbox label="Enable autopilot (checks every 60s)" defaultChecked={policy?.enabled} {...register("enabled")} />
          </label>
...
function NumberField(props: Omit<React.InputHTMLAttributes<HTMLInputElement>, "size"> & { label: string }) {
  const { label, ...rest } = props;
  return <Field label={label}><Input type="number" step="any" {...rest} /></Field>;
}
