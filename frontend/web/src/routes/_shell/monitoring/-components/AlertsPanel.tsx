...
export function AlertsPanel() {
  ...
  const addRule = async () => {
    ...
    if (!name.trim() || !isFinite(t)) {
      add({
        title: "Invalid rule",
        description: "Fill in a name and a valid threshold.",
        tone: "error",
      });
      return;
    }
  ...
  return (
    <Card>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-sm mb-sm">
        <Input
          leadingIcon={Tag as unknown as DesignIcon}
        ...
        />
        <Input
          placeholder={METRIC_HINT[metric]}
          value={threshold}
      ...
      <div className="space-y-sm mb-lg">
        {(rules ?? []).map((r) => (
          ...
          >
            ...
            <span className="font-code-md text-code-md text-on-surface-variant/60">
              {r.target_app ? "targeted" : "all services"}
            </span>
            <Button
              variant="ghost"
              onClick={() => deleteRule.mutateAsync(r.id)}
            >
              <Trash size={16} />
            </Button>
          </div>
        ...
        {(rules ?? []).length === 0 && (
          <EmptyState
            ...
            description="Add one above, for example CPU above 80%."
      ...
      <div className="space-y-sm max-h-[300px] overflow-y-auto sidebar-scroll">
        {(events ?? []).map((e) => (
          <div
            key={e.id}
            className={`flex items-start gap-sm rounded-xl border p-sm ${e.resolved_at ? "border-outline-variant/40 opacity-60" : "border-error/30 bg-error/5"}`}
          >
            {e.resolved_at ? (
              <Check size={16} className="mt-0.5 text-muted-foreground" />
            ) : (
              <XCircle size={16} className="mt-0.5 text-status-danger" />
            )}
            <div className="flex-1 min-w-0">
              ...
              <p className="font-code-md text-code-md text-on-surface-variant/60">
                {METRIC_LABEL[e.metric] ?? e.metric} {e.value.toFixed(1)} /{" "}
                {e.threshold} · {new Date(e.created_at).toLocaleString()}
