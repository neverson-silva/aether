...
export function BackupTab({ dbId, dbName }: { dbId: string; dbName?: string }) {
  ...
  const runNow = (configId: string) =>
    backupNow.mutate(configId, {
      ...
      onError: (e) => add({ title: "Backup failed", description: e instanceof Error ? e.message : "Unable to queue backup.", tone: "error" }),
    });
    ...
    return (
      <div className="space-y-lg">
        <Card className="p-xl text-center">
          <CloudArrowUp size={48} className="mx-auto text-on-surface-variant/40" />
  ...
  return (
    <div className="space-y-lg">
      <div className="flex items-center justify-between gap-md">
        ...
        <Button icon={designIcon(Plus)} onClick={() => { setEditingConfig(null); setConfigOpen(true); }}>Add schedule</Button>
      </div>
      {configs.map((cfg) => (
          ...
          <div className="flex flex-wrap items-start justify-between gap-md">
            <div className="min-w-0">
              <div className="flex items-center gap-sm mb-sm">
                ...
                <Badge tone={cfg.enabled ? "success" : "neutral"} dot>{cfg.enabled ? "Active" : "Disabled"}</Badge>
              </div>
            ...
            <div className="flex items-center gap-sm shrink-0">
              <Button variant="outline" size="sm" icon={designIcon(Gear)} onClick={() => { setConfigOpen(true); setEditingConfig(cfg); }}>Edit</Button>
              <Button variant="ghost" size="sm" icon={designIcon(Trash)} onClick={() => { setDeleteTarget(cfg); setDeleteOpen(true); }}>Remove</Button>
              <Button icon={designIcon(CloudArrowUp)} onClick={() => runNow(cfg.id)} loading={backupNow.isPending}>Backup now</Button>
            </div>
          </div>
        </Card>
      ))}
            ...
            {(backups ?? []).map((b) => {
              ...
              return (
                <div key={b.id} className="flex flex-wrap items-center gap-md px-lg py-md hover:bg-surface-container/50 transition-colors">
                  <Badge tone={b.status === "completed" ? "success" : b.status === "failed" ? "danger" : "neutral"} dot live>{backupTone(b.status).status}</Badge>
                  ...
                  <div className="flex items-center gap-sm shrink-0">
                    {active && (
                      <Button
                        ...
                        onClick={() =>
                          cancel.mutate(b.id, {
                            onSuccess: () => add({ title: "Cancellation requested", tone: "success" }),
                            onError: (e) => add({ title: "Cancellation failed", description: e instanceof Error ? e.message : "Unable to cancel backup.", tone: "error" }),
                          })
                        }
                      >
                        Cancel
                    ...
                    {b.status === "completed" && (
                      <Button variant="outline" size="sm" icon={designIcon(ArrowUUpLeft)} onClick={() => setRestoreTarget(b)}>
                        Restore
                      </Button>
                    )}
                  </div>
        ...
        <AlertDialog
          ...
          onConfirm={() =>
            deleteTarget && removeConfig.mutate(deleteTarget.id, {
              onSuccess: () => add({ title: "Backup configuration removed", tone: "success" }),
              onError: (e) => add({ title: "Remove failed", description: e instanceof Error ? e.message : "Unable to remove configuration.", tone: "error" }),
            })
          }
