  ...
  return (
    <main className="mx-auto flex w-full max-w-screen-2xl flex-col gap-8 p-6 lg:p-8">
      <PageHeader
        eyebrow="Security"
        title="Secrets vault"
        description="Secret variables encrypted at rest with AES-256-GCM. Values are never stored in plaintext."
      />
      ...
      <div className="grid grid-cols-1 gap-lg xl:grid-cols-2">
        <Card>
          ...
          <div className="space-y-sm">
            {(apps ?? []).map((app) => (
              <AppSecrets key={app.id} appId={app.id} name={app.name} />
            ))}
            {!apps?.length && (
              <EmptyState
                icon={LockKey as unknown as DesignIcon}
                title="No applications"
                description="Create an application to manage encrypted secrets."
              />
            )}
          </div>
        </Card>
        ...
        <Card>
          ...
          <p className="font-body-sm text-body-sm text-on-surface-variant/70 pt-sm">
            DEK rotation re-encrypts values without exposing plaintext.
            Enterprise: KEK in TPM/HSM.
          </p>
        </Card>
      </div>
    </main>
