    ...
    <main className="mx-auto flex w-full max-w-screen-2xl flex-col gap-lg p-6 lg:p-8">
      <header className="relative isolate overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-primary/10 via-surface-card to-secondary/5 px-lg py-xl shadow-sm sm:flex sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-sm text-body-md text-on-surface-variant">
            Automation keys for API v1. Each value is shown only once.
          </p>
        </div>
        <Dialog
          open={open}
          ...
          trigger={<Button icon={Key as unknown as DesignIcon}>New key</Button>}
        >
          <div className="space-y-5">
            {created ? (
              <>
                ...
                <CodeBlock code={created} language="API key" />
              ...
              <form onSubmit={form.handleSubmit(submit)} className="space-y-5">
                <Field label="Name" error={form.formState.errors.name?.message}>
                  <Input
                    leadingIcon={Key as unknown as DesignIcon}
                ...
                <Field
                  label="Scopes"
                  description="Comma-separated. Empty uses role permissions."
                >
                  <Input
                    placeholder="app.deploy, app.read"
      ...
      <Card variant="elevated" padding="none">
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="border-b border-border text-label-caps text-muted-foreground">
              <tr>
                {["Name", "Scopes", "Created at", ""].map((header) => (
            ...
            <tbody className="divide-y divide-border">
              {query.isLoading ? (
                ...
                (query.data ?? []).map((key) => (
                  <tr key={key.id} className="hover:bg-surface-container">
                    <td className="px-3 py-3 text-foreground">{key.name}</td>
                    ...
                    <td className="px-3 py-3 font-mono text-code-md text-muted-foreground">
                      {new Date(key.created_at).toLocaleDateString("en")}
                    </td>
        ...
        {!query.isLoading && !query.data?.length ? (
          <EmptyState
            icon={Key as unknown as DesignIcon}
            title="No API keys"
            description="Create a key for automation and integrations."
          />
        ) : null}
      </Card>
    </main>
