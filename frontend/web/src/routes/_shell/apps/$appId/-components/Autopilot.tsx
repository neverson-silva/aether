...
export function Autopilot({ appID }: { appID: string }) {
  ...
  if (isLoading)
    return (
      <div className="space-y-sm" aria-label="Loading autopilot settings">
  ...
  const submit = async (values: z.infer<typeof schema>) => {
    try {
      ...
      add({ title: "Policy saved", tone: "success" });
      setOpen(false);
    } catch (err) {
      add({
      ...
      {!open ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-md">
          ...
          <div>
            ...
            <div className="font-code-md text-code-md text-on-surface">
              {policy?.cpu_min ?? 0.25}–{policy?.cpu_max ?? 4}
            </div>
          </div>
          {!!events?.length && (
            <div className="col-span-2 md:col-span-4">
              <div className="font-label-caps text-label-caps text-on-surface-variant uppercase mb-sm">
                Recent actions
              </div>
              <div className="space-y-xs">
                {events.slice(0, 5).map((e) => (
                  <div
                    key={e.id}
                    className="flex flex-wrap items-center justify-between gap-sm rounded-xl border border-outline-variant/40 p-sm"
                  >
        ...
        >
          <label className="flex items-center gap-sm col-span-4 cursor-pointer select-none">
            <Checkbox
              label="Enable autopilot (checks every 60s)"
          ...
          <NumberField
            label="CPU min"
            {...register("cpu_min")}
            defaultValue={policy?.cpu_min}
          />
          <NumberField
            label="CPU max"
            {...register("cpu_max")}
            defaultValue={policy?.cpu_max}
          />
          <NumberField
            label="Mem min (MiB)"
            {...register("mem_min_mb")}
            defaultValue={policy?.mem_min_mb}
          />
          <NumberField
            label="Mem max (MiB)"
            {...register("mem_max_mb")}
            defaultValue={policy?.mem_max_mb}
          />
          <NumberField
            label="Scale up above %"
            {...register("scale_up_pct")}
            defaultValue={policy?.scale_up_pct}
          />
          <NumberField
            label="Scale down below %"
            {...register("scale_down_pct")}
            defaultValue={policy?.scale_down_pct}
          />
          <NumberField
            label="Cooldown (min)"
            {...register("cooldown_min")}
            defaultValue={policy?.cooldown_min}
          />
