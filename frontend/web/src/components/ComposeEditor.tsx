...
export function highlightYAML(line: string): React.ReactNode[] {
  ...
  for (const part of parts) {
    ...
    if (kw && trimmed.length <= 30) {
      out.push(
        <span key={out.length} className="text-primary">
          {part}
        </span>,
      );
    } else if (trimmed.startsWith("${")) {
      out.push(
        <span key={out.length} className="text-status-warning">
          {part}
        </span>,
      );
    } else if (/^#/.test(trimmed)) {
      out.push(
        <span key={out.length} className="text-on-surface-variant/40 italic">
          {part}
        </span>,
      );
    } else {
      out.push(
        <span key={out.length} className="text-on-surface">
          {part}
        </span>,
      );
    }
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-md">
      ...
      <div className="space-y-md">
        <div className="rounded-2xl border border-outline-variant bg-surface-container-low p-md shadow-sm">
          <p className="font-label-caps text-label-caps text-on-surface-variant uppercase mb-sm">
            Validation
          </p>
          {validation.isLoading && (
            <div className="space-y-sm" aria-label="Validating compose file">
          ...
          {validation.data && validation.data.valid && (
            <div className="space-y-sm">
              <p className="font-body-sm text-body-sm text-status-success flex items-center gap-sm">
              ...
              <div className="space-y-sm">
                {(validation.data.services ?? []).map((svc) => (
                  ...
                  >
                    ...
                    <span className="font-code-md text-code-md text-on-surface-variant/60 truncate max-w-[180px]">
                      {svc.image ||
                        (svc.build ? "build:" + svc.build.slice(0, 40) : "—")}
                    </span>
                  </div>
                ))}
              </div>
              {validation.data.total_ports > 0 && (
                <p className="font-code-md text-code-md text-on-surface-variant/60">
                  {validation.data.total_ports} exposed port(s)
                </p>
              )}
            </div>
          )}
          {validation.data && !validation.data.valid && (
            <div className="space-y-sm">
              {(validation.data.errors ?? []).map((e, i) => (
                <p
                  key={i}
                  className="font-body-sm text-body-sm text-error flex items-start gap-sm"
                >
            ...
            (validation.data.warnings ?? []).length > 0 && (
              <div className="space-y-sm mt-sm border-t border-outline-variant pt-sm">
                {validation.data.warnings.map((w, i) => (
                  <p
                    key={i}
                    className="font-body-sm text-body-sm text-status-warning flex items-start gap-sm"
                  >
                    <Warning
                      size={16}
                      className="shrink-0"
        ...
        <div className="rounded-2xl border border-outline-variant bg-surface-container-low p-md shadow-sm">
          ...
          {(validation.data?.services ?? []).map((svc) => (
            <div
              key={svc.name}
              className="flex items-center gap-sm font-code-md text-code-md py-1"
            >
              <Database size={14} className="text-primary" aria-hidden="true" />
              <span className="text-on-surface">{svc.name}</span>
              {svc.image && (
                <span className="text-on-surface-variant/60 truncate max-w-[140px]">
                  ← {svc.image}
                </span>
              )}
            </div>
          ...
          {(validation.data?.services ?? []).length === 0 && (
            <p className="font-body-sm text-body-sm text-on-surface-variant">
              Dependencies appear here as you type.
            </p>
          )}
        </div>
      </div>
