...
function StatusChip({ text }: { text: string }) {
  return (
    <span className="rounded-lg border border-primary/40 px-2 py-1 font-code-md text-code-md text-primary">
      {text}
    </span>
  );
}
...
export function CreateServiceModal({
  ...
  const detectFramework = async () => {
    ...
    try {
      ...
      add({
        title: err instanceof Error ? err.message : "Detection failed",
        tone: "error",
      });
    } finally {
      setDetecting(false);
  ...
  const submit = async (values: CreateForm) => {
    try {
      if (values.source_type === "compose") {
        await createCompose.mutateAsync({
      ...
      await createApp.mutateAsync({
        ...
        payload: {
          ...
          source_type:
            values.source_type === "template" ? "image" : values.source_type,
          image: values.image || "",
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit(submit)} className="space-y-lg" noValidate>
        ...
        {source === "template" ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-sm max-h-[240px] overflow-y-auto sidebar-scroll">
              {(trendingTemplates ?? []).slice(0, 18).map((t) => (
                <button
                        ...
                        : t.name.toLowerCase().includes("redis")
                          ...
                          : t.name.toLowerCase().includes("mysql")
                            ...
                            : t.name.toLowerCase().includes("mongo")
                              ? "mongo:7"
                              : t.name.toLowerCase().includes("nginx")
                                ? "nginx:alpine"
                                : "ghcr.io/" +
                  ...
                  className="flex items-center gap-sm rounded-xl border border-outline-variant bg-surface-container-low p-sm text-left outline-none transition-[background-color,border-color,box-shadow] duration-150 hover:border-primary/50 hover:bg-surface-container focus-visible:ring-2 focus-visible:ring-ring"
                >
                  <span className="text-[22px]">{t.icon}</span>
                  <span className="min-w-0">
                    <span className="block font-body-sm text-body-sm text-on-surface truncate">
                      {t.name}
                    </span>
                    <span className="block font-code-md text-code-md text-on-surface-variant/60 truncate">
                      {t.description}
                    </span>
                  </span>
                </button>
          ...
          <div className="relative">
            <Field
              ...
              description={
                !errors.image?.message ? "ex: nginx:alpine" : undefined
              }
            >
              <Input
                placeholder="registry/repo:tag"
                {...register("image")}
                onFocus={() => setImageSuggest(true)}
                onBlur={() => setTimeout(() => setImageSuggest(false), 150)}
              />
            </Field>
            {imageSuggest && (
              <div className="absolute z-10 mt-1 w-full overflow-hidden rounded-2xl border border-outline-variant bg-surface-modal py-1 shadow-xl">
        ...
        {detected && (
          <div className="space-y-sm rounded-2xl border border-primary/30 bg-surface-container-low p-md shadow-sm">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-md">
              <div>
                ...
                <p className="font-code-md text-code-md text-on-surface">
                  {detected.build_method || "buildpacks"}
                </p>
              </div>
              <div>
                <p className="font-label-caps text-label-caps text-on-surface-variant/60 uppercase">
        ...
        <details className="group rounded-2xl border border-outline-variant bg-surface-card shadow-sm">
          ...
          <div className="px-md pb-md space-y-lg border-t border-outline-variant/60 pt-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-lg">
              <Field
                label="Memory (MiB)"
                error={errors.mem_mb?.message}
                description={
                  !errors.mem_mb?.message ? "0 = unlimited" : undefined
                }
              >
              ...
              <Field
                ...
                error={errors.health_path?.message}
              >
            ...
            <label className="flex items-center gap-sm cursor-pointer select-none">
              ...
              <span className="font-body-md text-body-md text-on-surface">
                Enable health check (required for auto-rollback)
              </span>
            </label>
          </div>
        </details>
