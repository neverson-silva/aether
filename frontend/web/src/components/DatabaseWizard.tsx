...
export const DB_ENGINES: {
...
}[] = [
  {
    value: "postgres",
    label: "PostgreSQL",
    tagline: "Relational SQL",
    icon: "postgres",
    versions: ["17", "16", "15", "14", "latest"],
  },
  {
    value: "mysql",
    label: "MySQL",
    tagline: "Relational SQL",
    icon: "mysql",
    versions: ["8.4", "8.0", "8.4.3", "latest"],
  },
  {
    value: "mariadb",
    label: "MariaDB",
    tagline: "MySQL compatible",
    icon: "mariadb",
    versions: ["11.4", "11.6", "10.11", "latest"],
  },
  {
    value: "mongodb",
    label: "MongoDB",
    tagline: "NoSQL document",
    icon: "mongodb",
    versions: ["7.0", "6.0", "7.0.16", "latest"],
  },
  {
    value: "redis",
    label: "Redis",
    tagline: "In-memory datastore",
    icon: "redis",
    versions: ["7.2", "7.4", "6.2", "latest"],
  },
  {
    value: "mssql",
    label: "SQL Server",
...
const schema = z.object({
  ...
  user: z
    ...
    .optional()
    .refine(
      (v) => !v || /^[a-zA-Z][a-zA-Z0-9_]{0,62}$/.test(v),
      "lowercase/numbers/underscore, 63 max",
    ),
  password: z
    .string()
    .optional()
...
export function DatabaseWizard({
  ...
  const {
    ...
    defaultValues: {
      ...
      engine: (DB_ENGINES.find((e) => e.value === initialEngine)?.value ??
        "postgres") as FormValues["engine"],
      version: "",
    },
  });
  const engineValue = watch("engine");
  const selected =
    DB_ENGINES.find((e) => e.value === engineValue) ?? DB_ENGINES[0];
  ...
  const submit = async (values: FormValues) => {
    try {
      ...
      add({
        title: err instanceof Error ? err.message : "Failed to create database",
        tone: "error",
      });
    }
  };
    ...
    >
      <form onSubmit={handleSubmit(submit)} className="space-y-lg" noValidate>
        <div>
          ...
          {errors.engine && (
            <p className="font-body-sm text-body-sm text-error mb-sm">
            ...
            {DB_ENGINES.map((eng) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    setValue("engine", eng.value as never, {
                      shouldValidate: true,
                    });
                    setValue("version", "", { shouldValidate: true });
                  }}
                  className={`flex flex-col gap-sm rounded-xl border p-md text-left transition-[background-color,border-color,box-shadow,transform] duration-200 hover:-translate-y-px hover:shadow-sm ${active ? "border-primary bg-primary/10 shadow-sm" : "border-outline-variant bg-surface-container-low hover:border-primary/40"}`}
                >
                  <div className="flex items-center justify-between">
                    ...
                    />
                    <span
                      className={`w-2 h-2 rounded-full ${active ? "bg-green-400" : "bg-outline-variant"}`}
                    />
                  </div>
                  <div>
                    ...
                    <p className="font-code-md text-code-md text-on-surface-variant/70">
                      {eng.tagline}
                    </p>
                  </div>
                </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-lg">
          ...
          <Field
            ...
            description={
              !errors.user?.message ? "Empty = aether (auto)" : undefined
            }
          >
          ...
          <Field
            ...
            description={
              !errors.password?.message ? "Empty = auto-generated" : undefined
            }
          >
