...
export function useStudioRenameTable(dbId: string) {
  return useMutation({
    mutationFn: ({
      ...
      apiPost<StudioExecResult>(`/api/v1/databases/${dbId}/studio/tables/rename`, {
        schema,
        table,
        name,
      }),
...
export function useStudioDropTable(dbId: string) {
  return useMutation({
    mutationFn: ({ schema, table }: { schema: string; table: string }) =>
      apiPost<StudioExecResult>(`/api/v1/databases/${dbId}/studio/tables/drop`, {
        schema,
        table,
      }),
  })
...
export function useStudioAlterTable(dbId: string) {
  return useMutation({
    mutationFn: ({
      ...
      apiPost<StudioExecResult>(`/api/v1/databases/${dbId}/studio/tables/alter`, {
        schema,
        table,
        columns,
      }),
