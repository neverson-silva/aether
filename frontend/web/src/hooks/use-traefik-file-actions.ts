...
export function useSaveTraefikFile() {
  ...
  return useMutation({
    mutationFn: (input: { path: string; content: string }) => apiPut("/api/v1/admin/traefik/file", input),
    onSuccess: (_, input) => {
...
export function useDeleteTraefikFile() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (path: string) => apiDelete(`/api/v1/admin/traefik/file?path=${encodeURIComponent(path)}`),
