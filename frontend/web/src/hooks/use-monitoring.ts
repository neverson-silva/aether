    ...
    const consume = async () => {
      while (active) {
        try {
          ...
          if (!response.ok)
            throw new Error(`Monitoring stream failed with HTTP ${response.status}`)
          if (!response.body) throw new Error('Monitoring stream returned no body')
          const contentType = response.headers.get('content-type') || ''
          if (!contentType.toLowerCase().includes('text/event-stream'))
            throw new Error('Monitoring stream returned an invalid content type')
