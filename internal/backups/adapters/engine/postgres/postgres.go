...
func (a *Adapter) Restore(ctx context.Context, db application.DBDescriptor, src io.Reader) error {
	args := []string{"pg_restore", "-U", db.User, "-d", db.DBName, "--no-owner", "--no-privileges", "--exit-on-error", "-"}
	return a.exec.ExecIn(ctx, db.ContainerID, pgEnv(db.Password), src, io.Discard, io.Discard, args...)
}
