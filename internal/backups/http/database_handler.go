...
func (h *DBBackupHandler) GetConfiguration(c *gin.Context) {
	...
	cfg, err := h.svc.GetConfiguration(c.Request.Context(), dbID, orgID(c))
	if err != nil {
		abort(c, err)
		return
	}
...
func (h *DBBackupHandler) UpsertConfiguration(c *gin.Context) {
	if !h.manage(c) {
		return
	}
	dbID, ok := h.dbID(c)
	...
	saved, err := h.svc.UpsertConfiguration(c.Request.Context(), orgID(c), cfg)
	if err != nil {
		abort(c, err)
		return
	}
...
func (h *DBBackupHandler) DeleteConfiguration(c *gin.Context) {
	...
	if err := h.svc.DeleteConfiguration(c.Request.Context(), dbID, orgID(c)); err != nil {
		abort(c, err)
		return
	}
...
func (h *DBBackupHandler) CreateBackup(c *gin.Context) {
	...
	job, err := h.svc.StartManualBackup(c.Request.Context(), dbID, orgID(c))
	if err != nil {
		abort(c, err)
		return
	}
