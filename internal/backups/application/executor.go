...
func (s *DatabaseBackups) runBackup(ctx context.Context, orgID uuid.UUID, jobID uuid.UUID) {
	...
	cfg, err := s.Store.GetConfiguration(ctx, job.DatabaseID)
	if err != nil || cfg == nil {
		_ = s.failJob(ctx, job, "BACKUP_INVALID_CONFIGURATION", "no backup configuration")
		return
	}

	lock, ok, err := s.Locks.Acquire(ctx, "db:"+job.DatabaseID.String()+":backup", s.timeout())
	if err != nil {
		_ = s.failJob(ctx, job, "BACKUP_LOCK_FAILED", err.Error())
		return
	}
	if !ok {
		_ = s.failJob(ctx, job, "BACKUP_ALREADY_RUNNING", "another backup is running")
		return
	}
	...
	if err != nil {
		_ = s.failJob(ctx, job, "BACKUP_DATABASE_UNAVAILABLE", err.Error())
		return
	}
	pass, err := s.Passwords.Decrypt(db.PassEnc)
	if err != nil {
		_ = s.failJob(ctx, job, "BACKUP_DATABASE_UNAVAILABLE", "cannot decrypt credentials")
		return
	}
	desc := DBDescriptor{
	...
	if err != nil {
		_ = s.failJob(ctx, job, "BACKUP_UNSUPPORTED_ENGINE", err.Error())
		return
	}
	if err := adapter.Validate(ctx, desc); err != nil {
		_ = s.failJob(ctx, job, "BACKUP_TOOL_NOT_AVAILABLE", err.Error())
		return
	}
	...
	if err != nil {
		_ = s.failJob(ctx, job, "BACKUP_COMMAND_FAILED", err.Error())
		return
	}
	...
	if err != nil {
		_ = s.failJob(ctx, job, "BACKUP_STORAGE_UPLOAD_FAILED", err.Error())
		return
	}
	key, err := StorageKey(cfg.PathPrefix, job.DatabaseID, job.Engine, job.ID, time.Now(), job.Format)
	if err != nil {
		_ = s.failJob(ctx, job, "BACKUP_INVALID_CONFIGURATION", err.Error())
		return
	}
	job.StorageKey = key
	...
	if _, err := provider.HeadObject(ctx, storage.HeadObjectInput{Key: key}); err != nil {
		_ = s.failJob(ctx, job, "BACKUP_STORAGE_VERIFY_FAILED", err.Error())
		return
	}
...
func (s *DatabaseBackups) runRestore(ctx context.Context, orgID uuid.UUID, jobID uuid.UUID) {
	...
	path := file.Name()
	hash := sha256.New()
	if _, err := io.Copy(io.MultiWriter(file, hash), obj.Body); err != nil {
		_ = file.Close()
		_ = obj.Body.Close()
	...
	_ = rj.Transition(domain.RestoreRunning)
	_, _ = s.Store.UpdateRestoreJob(ctx, rj)
	...
	_ = rj.Transition(domain.RestoreVerifying)
	completed := time.Now()
	rj.CompletedAt = &completed
	_ = rj.Transition(domain.RestoreCompleted)
	_, _ = s.Store.UpdateRestoreJob(ctx, rj)
