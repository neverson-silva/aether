...
func (s *DatabaseBackups) StartManualBackup(ctx context.Context, dbID, orgID uuid.UUID) (*domain.BackupJob, error) {
	...
	if err := s.enqueue(ctx, queue.Job{ID: job.ID.String(), Type: "backup", OrgID: orgID.String(), Payload: []byte(job.ID.String())}); err != nil {
		_ = s.failJob(ctx, job, "BACKUP_ENQUEUE_FAILED", err.Error())
		return nil, err
	}
...
func (s *DatabaseBackups) failJob(ctx context.Context, job *domain.BackupJob, code, msg string) error {
	now := time.Now()
	job.ErrorCode = code
