...
func (s *DatabaseBackups) RestorePreflight(ctx context.Context, backupID, targetDBID, orgID uuid.UUID) (*PreflightResult, error) {
	...
	if err != nil {
		...
		if _, err := provider.HeadObject(ctx, storage.HeadObjectInput{Key: backup.StorageKey}); err != nil {
	...
	if engineOK {
		...
		if err == nil {
			...
			if adapter, aerr := adapterForEngine(desc.Engine); aerr == nil {
				if verr := adapter.Validate(ctx, desc); verr != nil {
